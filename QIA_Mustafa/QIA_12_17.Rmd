...
In this file, all the variables with suffix **_CAT** is the categorized continuous variables by the following rules:
...
2)	Interpretation for HER2 (IHC) TESTING IN BREAST CANCER:
...
```{r table2, echo = F}
knitr::kable(summ_dich, format = "markdown", caption = "Table 2: Summary statistics of dichotomized ER_HUAMN and PgR_HUMAN")
```
...
#HER2_HUMAN vs HER2_COREO (both ordinal cat --- stuart tau-c)
...
QIA$HER2_COREO_CAT <- cut(as.numeric(QIA$HER2_COREO)-1, breaks = c(-1,1.5,2.5,3.5),
...
* ER_HUMAN vs ER_COREO (both continuous --- interclass correlation coefficient(ICC))

The concordance between ER_HUMAN and ER_COREO is **`r round(corr_ER$value, 2)`**(p=`r round(corr_ER$p.value,2)`) with corresponding 95% Confidence Interval (`r round(corr_ER$lbound,2)`, `r round(corr_ER$ubound,2)`).

The scatter plot is shown below

```{r p1, echo=F, out.width="80%", fig.align="center", warning=F}
ggplot(QIA, aes(x=ER_HUMAN, y=ER_COREO)) + 
  geom_point() +
  ylab("Estrogen receptor digital image analysis score") +
  xlab("Estrogen receptor pathologists' score")
```
...
```{r p2, echo=F, out.width="80%", fig.align="center", warning=F}
ggplot(QIA, aes(x=PgR_HUMAN, y=PgR_COREO)) + 
  geom_point() +
  ylab("Progesteron receptor digital image analysis score") +
  xlab("Progesteron receptor pathologists' score")
```

* HER2_HUMAN vs HER2_COREO (both categorical --- KAPPA / weighted KAPPA)

The concordance between HER2_HUMAN_CAT and HER2_COREO_CAT is **`r round(KAPPA_2$kappa,2)`** (Kappa) or **`r round(KAPPA_2$weighted.kappa,2)`** (Weighted Kappa). 
...
The result shows that p-value = 0.3183 > 0.05, which means there is no evidence to show that they are significantly correlated.

#### Aim 3B
Correlation between HER2_COREO and HER2D17Z1_RATIO_CAT (both categorical with small sample size --- Fisher's exact test)
...
The result shows that p-value = 0.2734 > 0.05, which means there is no evidence to show that they are significantly correlated.
...
The result shows that p-value = 0.1281 > 0.05, which means there is no evidence to show that they are significantly correlated.
...
The contingency table is your **Table 1**.

* PgR_HUMAN_CAT vs PgR_COREO_CAT (both categorical --- weighted KAPPA)

The concordance between PgR_HUMAN_CAT and PgR_COREO_CAT is **`r round(corr_PgR_CAT$kappa,2)`** (Kappa) or **`r round(corr_PgR_CAT$weighted.kappa,2)`** (weighted Kappa). 
